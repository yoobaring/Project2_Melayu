...
public class MostrarActivity extends AppCompatActivity implements IAxiliarPersona {
    ...
    public void OpcionEliminar(final Persona persona) {
        ...
        alerta.setTitle("คำศัพท์");
        alerta.setMessage("คุณแน่ใจหรือว่าต้องการลบ?" + persona.getNombre() + " " + persona.getApellidos());
        alerta.setCancelable(false);
        alerta.setPositiveButton("ยืนยัน", new DialogInterface.OnClickListener() {
            @Override
            public void onClick(DialogInterface dialog, int which) {
                eliminarPersona(persona);
            }
        });
        alerta.setNegativeButton("ยกเลิก", new DialogInterface.OnClickListener() {
            @Override
            public void onClick(DialogInterface dialog, int which) {
    ...
    private void eliminarPersona(Persona persona) {
        ...
        if (!codigo.isEmpty()) {
            ...
            Toast.makeText(this, "นำออกเรียบร้อย", Toast.LENGTH_SHORT).show();
            personaAdapter.eliminarPersona(persona);
            sqLiteDatabase.close();
        } else {
            Toast.makeText(this, "ไม่สามารถลบข้อมูล ", Toast.LENGTH_SHORT).show();
        }
